...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if not (iteration < opt.mask_start or iteration > opt.mask_stop):
            if iteration > opt.stop_MLP:
                freeze_grad(gaussians, name = "deform", state = False)
            freeze_grad(gaussians, name = "msk_f", state = True)
            ...
            if iteration >= opt.warm_up and iteration <= opt.mask_start:
            ...
            if iteration > opt.stop_MLP:
                freeze_grad(gaussians, name = "deform", state = False)
            freeze_grad(gaussians, name = "msk_f", state = False)
...
if __name__ == "__main__":
    ...
    args.mask_start = 30_000
    args.mask_stop = 40_000
